...
-- RemoteEvent that tells a specific client how much wood was awarded.
...
-- To prevent trees from breaking faster than intended, track the last time
-- each player damaged a specific tree. If a player sends damage requests
-- more frequently than once per second, those extra requests are ignored.
...
local function awardWoodToPlayer(player: Player, amount: number)
    ...
    -- Notify the player client of the amount awarded so they can display a
    -- pickup notification. We send only the amount; the client can look up
    -- the log icon and display it as needed.
...
damageEvent.OnServerEvent:Connect(function(player: Player, treeModel: Instance)
    ...
    -- Rate limiting: ensure at least one second has passed since this
    -- player last damaged this tree. Without this check, multiple damage
    -- requests could slip through and fell a tree too quickly.
    local now = os.clock()
    ...
    if now - last < 1 then
        -- Too soon since last hit; ignore this hit
        return
    end
    ...
    if state.health <= 0 then
        ...
        -- Reset last hit records for this tree so that when it respawns
        -- players can immediately begin hitting it again.
